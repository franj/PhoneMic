...
- ✅ **隐私安全** – 局域网模式下数据只在局域网内传输，不经过任何云端服务器；Cloudflare 隧道模式下数据通过 HTTPS 隧道传输。需要更高安全性时，可在主界面菜单 **网络 → 加密方式** 中开启端到端加密（XSalsa20 / XChaCha20，NaCl crypto_box），密钥通过二维码传递、不经过网络，防止中间节点窃听。
...
3. 用手机扫码打开页面，手机端自动完成认证（开启端到端加密时会先完成密钥交换）。
4. 认证通过后即可正常使用。

### 说明

- **安全性：** 隧道本身提供 HTTPS 加密，二维码中携带连接令牌完成认证。开启端到端加密后，PC 公钥通过二维码带外传递，手机用 crypto_box_seal 密封自己的公钥发送给 PC，此后双向通信全部加密。
...
- **公司/单位网络：** 如果局域网直连都不通，说明网络管控较严。Cloudflare 隧道通过互联网中转，连接本身有 HTTPS 加密（可再开启端到端加密），但请确认单位是否允许建立外部隧道连接，使用前请确认是否违反单位安全规定。
