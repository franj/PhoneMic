...
- ✅ **隐私安全** – 通信**全程端到端加密**，密钥只在手机和电脑两端，中间节点看不到内容：局域网数据不经过云端，Cloudflare 则在 HTTPS 之上再叠加一层（**0.6.1 起所有模式一律加密**，此前局域网模式可以选择不加密）。认证方式可选 **手动审批**（局域网默认，首次在电脑上核对 4 位识别码并点「允许」，之后自动认证）或 **扫码认证**，在菜单 **网络** 里切换；两种都能扫码连接，手动审批也可以直接输入地址。
...
3. **连接手机**  
   ...
   - 不方便扫码时，也可在手机浏览器直接输入主界面显示的地址；若认证方式是「手动审批」（局域网默认），扫码和输入地址都一样，首次连接需在电脑上点一次「允许」（详见[加密与认证](USER_GUIDE.md#27-加密与认证)）。
