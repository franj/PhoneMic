...
def main():
    ...
    if dashboard.get_mode() == TunnelMode.CLOUDFLARE:
        ...
        dashboard._set_ip_text(i18n.tr("dashboard.cf_connecting"))
    ...
    def on_backend_event(event_type: str, payload: Any):
        if event_type == "preview":
            ...
            # 命中命令时先撤销 preview 阶段打出的字面文字再执行，否则「回车」
            # 这类文字会和命令的效果一起留在目标程序里
            if not command_interceptor.process_send_text(
                payload, before_execute=discard_preview
            ):
