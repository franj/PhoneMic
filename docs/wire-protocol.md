...
- 审批是**一条连接一个实例**、进队列后由用户按 id 结算（`e2ee-always-on-design.md` §7.4）：队列按注册先后排，界面显示**排队最久**的那条（新连接排到末尾，不顶掉正在核对的那条——队首那个数字在核对途中被换掉，用户就会为"自己看过的码"批准"另一条请求"）；同一 IP 的新连接取代该 IP 的旧请求（`superseded`）并**接管它的队列位置**（手机刷新页面＝原地换码，不换位置）；用户批准一条会作废其余待审批请求（`superseded`）；对端断开立刻摘除（`disconnected`）。这四个 reason 只是 close 帧的文案，**不改变线上帧格式**。
