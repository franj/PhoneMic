...
3. **暂停握手**，把这条申请登记进**审批队列**（一条连接 = 一个 `ApprovalRequest` 实例，见 §7.4），
   在 dashboard 主界面显示**排队最久**的那条：**同一个识别码** + 连接来源 IP + 接受/拒绝按钮。
   新连接排到末尾等着，不顶掉队首（理由见 §7.4「审批 UI」）。
4. 等待用户操作（等待的是**这一条**实例的 future，不是"当前那条"）：
   ...
   - **同 IP 来了新连接**（手机刷新页面 / 网络抖动重连） → 旧那条被取代（`reason="superseded"`），
     新连接并**接管它的队列位置**：界面上是原地换码，不换位置（见 §7.4）。
...
> **无痛显示，不弹窗**：审批通知在 dashboard 主界面的固定区域（地址栏/说明区那一块）显示，不使用模态弹窗。有人不断尝试连接时不会反复弹窗打扰用户——队列里排着，界面只显示队首那条（＝排队最久的那条，新连接不顶掉它），用户按自己的节奏处理；并发过多时在面板里加一行醒目文字（并发 ≥3 条时另有「全部拒绝」按钮），仍然不弹框：弹框会骚扰根本没在用程序的人。
>
...
界面侧只拿到**不可变快照** `{"items": [{"id","pin","ip","remaining"}, ...], "pending": N}`（排队最久的在前），
...
| 事件 | 对队列的影响 |
|---|---|
| 新请求登记 | 加入队列**末尾**（界面显示排队最久的那条，新连接不顶掉它）；**同 IP 的旧待审批请求被取代**（`superseded`），新连接**接管它的队列位置**（原地换码，不换位置） |
...
- **同 IP 重连是借位置、不占新位置**：新连接接管被取代者的队列位置，于是刷新页面时界面上那条**原地换码**、不往下跳一位。deadline / created 仍按**本次**连接重算，不继承旧的——手机端 35s 预算从它自己发 auth 那刻起算，继承旧 deadline 会让两边错位（服务端可能只剩 3s、手机还有 35s，用户核对完点「允许」时 PC 早已超时）。
- **队列按注册先后排**（`queue_seq`），界面只显示排队最久的那条。它不一定等于"这条连接什么时候来的"（同 IP 重连会借走旧位置），所以它只是一个位置号、不进快照，别拿它当时间用。
...
- **队首永远是排队最久的那条**，不是最新的那条；新连接只能排到后面等。这是安全约束而非观感：队首那个 4 位数字正被用户逐个核对，界面若在核对途中把它换成另一条请求的码，用户就会为"自己核对过的码"批准"另一条请求"——等于把人工审批这一步整个作废（用户按下的按钮提交的是队首的 id）。代价只是新连接多等一次结算（队首被处理后立刻轮到它），而不是让用户去猜屏幕上那个数字什么时候换了人。
...
- 审批队列（`tests/test_approval_registry.py`）：两条不同来源的请求同时排队（**先来的在前**，队首不被后到的顶掉）；批准一条 ⇒ 其余 superseded；拒绝一条 ⇒ 其余不受影响（下一条顶上来）；同一 id 重复点击不误伤新请求；同 IP 新连接取代旧请求（**接管其队列位置**，且不影响别的 IP）；刷新借位置但重算自己的 deadline；掉线/超时各自如实入账；服务停止清空队列
