...
- 识别码放在 auth 帧的 `pin` 字段中明文传输（攻击者能看到，但无法利用——见下）。
...
> 识别码不是密码学保护——它不防被动窃听（码在明文 auth 里）。它的作用是给**用户审批**提供一个可核对的凭据，类似蓝牙配对码。4 位纯数字碰撞概率为 1/10,000（10^4），在审批场景下足够——攻击者每次连接只猜一次，30 秒审批超时内成功率 0.01%。
